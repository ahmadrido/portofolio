...
export default function JSXAPP() {
  ...
  if (state.succeeded) {
    ...
    return <div className="thanks w-full h-screen" style={{overflow:'hidden', zIndex:'99999'}}>
  ...
  function handleClose() {
    document.getElementsByClassName('try')[0].style.display = 'none'
  }
...
<form className="w-full py-10 text-white" onSubmit={handleSubmit}>
  ...
  <ShinyButton className='text-white' disabled={state.submitting}>Send <i className='fa-solid fa-paper-plane'></i></ShinyButton>
    ...
    {/* alert coba mainkan id card nya */}

     <div className="try border mx-5 -mt-20 w-max p-3 rounded-xl bg-slate-600">
      <h1>try playing the id card <i className='fa-solid fa-close border p-2 bg-slate-400 rounded-lg cursor-pointer' onClick={handleClose}></i></h1>
      </div> 
...
function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  const bodyTexture = useTexture('https://i.ibb.co.com/mT55tXk/11w-1.png')
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          <group
            ...
            <mesh geometry={nodes.card.geometry}>
              <meshPhysicalMaterial useMap map={bodyTexture} map-anisotropy={16} clearcoat={1} clearcoatRoughness={0.15} roughness={0.3} metalness={0.5} />
            </mesh>
